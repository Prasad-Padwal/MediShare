...
function UserDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const donationId = await addDonation(fullDonationData, frontImage, backImage);
      ...
      setError(error.message || 'An error occurred while submitting the donation.');
    }
  };

  return (
    <div className="container mx-auto mt-10 p-8 bg-gray-50 rounded-xl shadow-lg animate-fade-in">
      <h1 className="text-4xl font-bold mb-8 text-blue-600 text-center">User Dashboard</h1>
      {error && <p className="text-red-500 mb-6 text-center bg-red-100 p-3 rounded-lg">{error}</p>}
      <form onSubmit={handleSubmit} className="space-y-6 max-w-md mx-auto">
        ...
        <div className="animate-slide-up" style={{animationDelay: '0.6s'}}>
          ...
          >
            {submitStatus === 'submitting' ? 'Submitting...' : 'Submit Donation'}
          </button>
        </div>
      </form>
      {submitStatus === 'success' && (
        <p className="text-green-500 mt-6 text-center bg-green-100 p-3 rounded-lg animate-fade-in">Donation submitted successfully!</p>
      )}
      {submitStatus === 'error' && (
        <p className="text-red-500 mt-6 text-center bg-red-100 p-3 rounded-lg animate-fade-in">Error submitting donation. Please try again.</p>
      )}
    </div>
  );
}
